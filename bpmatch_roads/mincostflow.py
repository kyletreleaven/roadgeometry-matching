...
def FragileMCCF( network, capacity, supply, cost, epsilon=None ) :
    ...
    while Delta >= epsilon :
        ...
        for resedge in rgraph.edges() :
            ...
            # b/c, we must allow re-linearization between augmentations
            while True :
        ...
        while True :
            ...
            CERT = { re : c for (re,c) in redcost.iteritems() if c < 0. }
            print 'CERTIFICATE: %s' % repr( CERT )
